...
const Login = () => {
  ...
  return (
    <div className="login force-margin">
      ...
      <p>
        {" "}
        Ukoliko želite da iznajmite motorna vozila, priložite i vozačku dozvolu pored lične karte. Ukoliko nastavite bez vozačke dozvole, za
        iznajmljivanje će biti dostupno samo nemotorna vozila.{" "}
      </p>
      <Button fullWidth variant="contained" onClick={() => navigate("/user")}>
        {" "}
        Dodaj vozačku{" "}
      </Button>
      <Button fullWidth onClick={() => navigate("/user")}>
        {" "}
        Nastavi bez vozačke{" "}
      </Button>
    </div>
